...
class ResolveHubShell:
            ...
            [self.ui.HGroup({"Spacing": 0, "StyleSheet": theme.ROOT}, [
                ...
                self.ui.VGroup({"Weight": 1, "Spacing": 7}, [
                    self.ui.HGroup({"Spacing": 7, "StyleSheet": theme.SURFACE, "Weight": 0}, [
                        ...
                        self.ui.Label({"ID": "ContextTimecode", "Text": "—", "StyleSheet": theme.SECTION, "MinimumSize": [105, 22], "MaximumSize": [105, 22], "Weight": 0}),
                    ...
                    self.ui.HGroup({"Spacing": 6, "StyleSheet": theme.STATUS, "Weight": 0}, [self.ui.Label({"ID": "StatusText", "Text": "Ready", "StyleSheet": theme.SUBTITLE, "MinimumSize": [120, 20], "MaximumSize": [520, 20], "Weight": 1}), self.ui.HGap(0, 1), self.ui.Label({"ID": "UndoText", "Text": "", "StyleSheet": theme.SUBTITLE, "MinimumSize": [100, 20], "MaximumSize": [280, 20], "Weight": 0}), button(self.ui, "UndoLast", "Undo")]),
                ]),
            ])],
        )
    ...
    def _refresh_selection(self):
        ...
        for identity, mode in style_ids.items(): self.window.GetItems()[identity].StyleSheet = theme.SELECTION_ACTIVE if mode == self.selection_mode else theme.SECONDARY
    ...
    def _refresh_markers(self, event=None, select_frame=None, context=None, records=None):
        visible_before = self._visible_markers() if self.filtered_markers else []
    ...
    def _handle_marker_clicked(self, event=None):
        self._thumbnail_idle_until = time.monotonic() + 0.25
        ...
        if item is not None:
            self._set_marker_item_editable(item, column not in (0, 2, 6))
        if marker:
    ...
    def _get_or_create_marker_thumbnail(self, marker):
        ...
        # Keep any current image visible until its replacement is ready.
        try:
