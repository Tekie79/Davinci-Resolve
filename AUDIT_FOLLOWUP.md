# Resolve Hub marker-audit follow-up — merged into v0.4.0
...
This audit does not claim that all P0 functionality or every Resolve build is fully validated. The existing disabled Play Range control remains disabled; unsupported API features are not simulated. No source camera files are renamed/deleted. The audited fixes were subsequently reconciled with the newer v0.4.0 main branch; live Resolve acceptance on macOS remains required.
