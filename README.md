...
DaVinci Resolve
  │
  ├── Workspace → Scripts → Meher Flow Resolve Hub
  │
  └── Resolve scripting API
          ↑
Installed runtime
~/Library/Application Support/Meher Flow/Resolve Hub/runtime/
  └── meher_resolve_hub/
      ├── app.py
      ...
      ├── speaker_markers.py
      └── services/
          ├── timeline_audio_export_service.py
          ├── openai_diarization_service.py
          └── speaker_marker_service.py
          ↑
Local MCP server
          ↑
Codex
...
The GitHub repository is the **development/update source**. Normal Codex use
calls the **installed runtime**, not the repository copy.
...
From your local Davinci-Resolve clone:

~~~bash
git pull
~~~

## 2. Install the Resolve Hub runtime and MCP environment
...
1. copies meher_resolve_hub to the installed runtime directory;
2. installs the Resolve menu launchers;
3. creates a dedicated MCP Python virtual environment;
4. installs the MCP/OpenAI runtime dependencies;
5. installs a local MCP launcher.
